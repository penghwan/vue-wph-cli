...
- 链接包到全局下使用
- 注意：在Mac环境下使用sudo npm link，在Windows环境下使用管理员打开cmd。
...
下载前先找个临时目录存放下载的文件。
我们将文件下载到当前用户的.template文件中，由于系统的不同目录获取方式不一样，process.platform在window下获取的是win32，我这里是mac所获取的值是drawin，再根据对应的环境变量获取到用户目录。
...
// 下载项目
const target = await waitFnloading(download, 'download template')(repo, tag);
```
...
## 3项目发布
```js
npm install --global vue-wph-cli
```

- 打开npmjs.com官网
- 注册一个npm账号
- 在npm检索是否有重命名的包名，包名为`package.json`中的`name`
- 将`package.json`中的`name`修改为发布到npm上的包名，注意：和本地项目名称无关
- 打开控制台，执行`npm login`，登录npm
- 登录成功以后，在项目下执行`npm publish`发布
- 发布成功后，就可以在本地进行安装测试了
- 本地测试流程：（1）取消关联本地脚手架：`npm unlink -g vue-wph-cli`；（2）安装从npm下载的脚手架：`npm install --global vue-wph-cli` 
